$('#acceptTrainerRequest').on('click', function(){
	var $el = $('#acceptTrainerRequest');
	var requestId = $el.data('request');
	console.log($el);
	
	$.ajax({
	  url: '/acceptTrainerRequest',
	  type: 'GET',
...
$('#discardTrainerRequest').on('click', function(e){
	var $el = $('#discardTrainerRequest');
	var requestId = $el.data('request');
	
	$.ajax({
	  url: '/discardTrainerRequest',
	  type: 'GET',
	  data: 'requestId=' + requestId,
	  contentType : 'application/json',
	  dataType: 'text',
		...
		if($comment.val()){
			$.ajax({
		        ...
		        success: function (data) {
		        	...
		        	var numberOfComments = parseInt($('#numberOfComments').text());
		        	$('#numberOfComments').text(numberOfComments + 1);
		        	var date = new Date();
